  ...
  return (
    <div className="mb-10 max-w-[20rem] rounded-2xl border border-zinc-200/70 bg-white/70 p-5 shadow-md shadow-zinc-200/40 backdrop-blur-sm md:mb-12 md:max-w-[22rem] md:p-6">
      <div className="flex items-start gap-5 md:gap-6">
        <div
          className="grid w-fit shrink-0 grid-cols-5 grid-rows-7 gap-1 md:gap-1.5"
          aria-hidden
        >
          {grid.flatMap((row, r) =>
            row.map((level, c) => (
              <div
                key={`${r}-${c}`}
                className={`size-[10px] rounded-[2px] md:size-3 md:rounded-sm ${levelClass[level]}`}
              />
            )),
          )}
        </div>
      ...
      {handle ? (
        <p className="mt-4 text-right md:mt-5">
          <a
