...
export default function GithubContributionCard({ summary, username }: Props) {
  ...
  return (
    <div className="mb-10 max-w-[20rem] rounded-2xl border border-zinc-200/70 bg-white/70 p-5 shadow-md shadow-zinc-200/40 backdrop-blur-sm md:mb-10 md:max-w-[20.5rem] md:p-5">
      <div className="flex gap-5 md:gap-5">
        {/* 줄 높이는 max(잔디, 오른쪽 콘텐츠). justify-end로 잔디를 아래 붙여 핸들(오른열 하단)과 한 줄에 맞춤 */}
        <div className="flex shrink-0 flex-col justify-end">
          <div
            className="grid w-fit grid-cols-5 grid-rows-7 gap-1 md:gap-1"
            aria-hidden
          >
            {grid.flatMap((row, r) =>
              row.map((level, c) => (
                <div
                  key={`${r}-${c}`}
                  className={`size-[10px] rounded-[2px] md:size-3.5 md:rounded-sm ${levelClass[level]}`}
                />
              )),
            )}
          </div>
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col justify-between text-left">
          <div className="pt-0.5">
            <p className="text-base font-bold leading-tight tracking-tight text-zinc-900 md:text-base">
              This year{" "}
          ...
          {handle ? (
            <p className="text-right">
              <a
                ...
                className="text-xs text-zinc-400 transition-colors hover:text-zinc-600 md:text-[0.6875rem]"
              >
                @{handle}
              </a>
            </p>
