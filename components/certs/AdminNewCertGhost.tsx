...
export function AdminNewCertGhostMobile({ disabled, onClick }: GhostProps) {
  return (
    <button
      type="button"
      disabled={disabled}
...
export function AdminNewCertGhostDesktop({ disabled, onClick }: GhostProps) {
  return (
    <button
      type="button"
      disabled={disabled}
      ...
      aria-label="새 자격증 추가"
    >
      <div className="flex min-h-[12rem] flex-1 flex-col items-center justify-center gap-2 px-4 py-6 md:min-h-0 md:gap-2 md:py-3">
        <span className={plusCircle} aria-hidden>
          <AiOutlinePlus
            className="h-[0.95rem] w-[0.95rem] md:h-4 md:w-4"
            strokeWidth={1.65}
